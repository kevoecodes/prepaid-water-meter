...
class MongoTransactionsManager(MongoDB):
    ...
    def create(self, data):
        try:
            ...
            data['type'] = data['type']
            self.transactions_coll.insert_one(data)
            return True
        except:
            return False
    ...
    def purchaseUnits(self, data):
        ...
        new_balance = float(account['current_balance']) - float(data['amount']) 
        self.create({"mobileNo": data['mobileNo'],"amount": data['amount'], "type": "Purchase"})         
        self.account_coll.update_one({"accountNo": user['accountNo']}, {"$set": {
    ...
    def conductTransaction(self):
        try:
            if self.user_account  is not None:
                ...
                {"current_balance": str(self.new_balance),
                "cash_used": str(self.total_cash_used),
                "consumed_litres": str(self.total_consumes_litres)}})
...
class MongoUnitsUpdate(MongoDB):
    ...
    def updateUnits(self):
        ...
        "consumed_litres": self.account['consumed_litres']}})
